...
EARLY_STOPPING_METRIC = "unit_test"  # "eval_loss" or "unit_test"
...
loss_history, lr_history, eval_loss_history, pass_rate_history = [], [], [], []
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % self.eval_every == 0:
            ...
            if pass_rate > self.best_pass_rate:
                ...
                logging.info("📀 New best model saved based on Pass Rate.")
                self.no_improve_count = 0
            else:
...
trainer = Trainer(
    ...
    callbacks=[
        LossLogger(),
        CustomEarlyStoppingCallback(eval_every=EVAL_EVERY, patience=PATIENCE)
    ],
    data_collator=DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False)
)

for cb in trainer.callback_handler.callbacks:
    if isinstance(cb, (CustomEarlyStoppingCallback, LossLogger)):
...
def save_list_to_csv(data, path, header):
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(data)

save_list_to_csv(loss_history, os.path.join(SAVE_PATH, "loss_history.csv"), ["step", "loss"])
save_list_to_csv(lr_history, os.path.join(SAVE_PATH, "lr_history.csv"), ["step", "learning_rate"])
save_list_to_csv(pass_rate_history, os.path.join(SAVE_PATH, "pass_rate_history.csv"), ["epoch", "pass_rate"])

# -------------------- PLOTS --------------------

def plot_metric(data, x_label, y_label, title, path):
    if data:
        x, y = zip(*data)
...
plot_metric(loss_history, "Step", "Loss", "Training Loss over Steps", os.path.join(SAVE_PATH, "loss_curve.png"))
plot_metric(lr_history, "Step", "Learning Rate", "Learning Rate over Steps", os.path.join(SAVE_PATH, "lr_curve.png"))
plot_metric(pass_rate_history, "Epoch", "Pass Rate", "Pass Rate over Epochs", os.path.join(SAVE_PATH, "pass_rate_curve.png"))

logging.info("📊 Training Complete. All models, plots, and CSVs saved!")
